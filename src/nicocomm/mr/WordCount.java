        ...
        Mapper<LongWritable, Text, Text, IntWritable> {
            ...
            @Override
                ...
                throws IOException, InterruptedException {
                    ...
                    line = line.trim();
                    try {
                        // lineは一行がJSONデータとなっている。
                        JSONObject jso = new JSONObject(line);
                        // jsoから、tagsタグがついたjsonオブジェクトを取得
                        //JSONObject tags = jso.getJSONObject("tags");
                        // ↑ をJSONArrayオブジェクトで取得したい
                        JSONArray tags = jso.getJSONArray("tags");
                        // tagsから順に、tagを取り出す。
                        for(int i=0; i<tags.length(); i++) {
                            JSONObject a_tag = tags.getJSONObject(i);
                            String tag = a_tag.getString("tag");
                            // すべて小文字にし、mapメソッドの出力キーとする
                            outKey.set(tag.toLowerCase());
